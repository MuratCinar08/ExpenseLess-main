...
def prepare_data(emails):
    """
    Convert the list of emails into a DataFrame, ensuring that date and total_amount
    are correctly formatted and converted to numeric values.
    """
...
def generate_plot(emails):
    df = prepare_data(emails)

    # Check if DataFrame is not empty after conversion
    if df.empty:
        return None
    ...
    # Save the plot to a BytesIO object and encode as base64 for rendering
    img = io.BytesIO()
    plt.tight_layout()
