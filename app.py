...
def fetch_email_data(service, keywords, year, month, sender_filter=None):
    """Fetch and process emails with a given filter (optional)."""
    emails, _ = list_emails_with_month(service, keywords, year, month, sender_filter)
    email_data = []
...
def generate_plot(data):
    """Generate pie chart plot for email data."""
    if data.empty:
        return None
...
def process_enriched_emails(service, emails, source):
    """Process and enrich emails from a specific source."""
    enriched_emails = []
    for email in emails:
        msg_data = service.users().messages().get(userId='me', id=email['id']).execute()
        payload = msg_data.get('payload', {})
...
def dashboard():
    ...
    trendyol_emails, _ = list_emails_with_month(service, trendyol_keywords, selected_year, selected_month, query="from:info@trendyolmail.com")
    other_emails, month_name = list_emails_with_month(service, general_keywords, selected_year, selected_month, query=f"({' OR '.join(general_keywords)})")

    enriched_emails = []
    enriched_emails.extend(process_enriched_emails(service, trendyol_emails, 'Trendyol'))
    enriched_emails.extend(process_enriched_emails(service, other_emails, 'Other'))
